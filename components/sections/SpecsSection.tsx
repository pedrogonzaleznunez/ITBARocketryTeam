...
export default function SpecsSection() {
  ...
  useEffect(() => {
    ...
    checkContentFit();
  }, []); // Solo se evalúa una vez al montar, no cambia entre pasos
    ...
    <div ref={sectionRef} className="relative min-h-screen bg-[#111727] text-white">
      <Section noPadding>
            ...
            <div className="flex flex-col items-center justify-center">
              {steps.map((_, index) => (
                <div key={index} className="flex flex-col items-center">
                  {/* Número */}
                  <div
                    className={`w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center text-lg sm:text-xl font-bold transition-all duration-500 ${
                      index === activeStep
        ...
        <div className="hidden lg:grid lg:grid-cols-2 gap-16 items-start min-h-screen pt-32 pb-12 lg:pt-36 lg:pb-16 overflow-visible">
          ...
          <div 
            ref={contentRef}
            className={`flex flex-col justify-center transition-all duration-300 overflow-visible ${
              needsCompactLayout ? 'space-y-6' : 'space-y-8'
            }`}
          >
            <div className="flex items-baseline gap-4">
              <span className={`font-bold text-primary-400 ${
                needsCompactLayout ? 'text-6xl' : 'text-7xl'
              }`}>
                {String(activeStep + 1).padStart(2, '0')}
              </span>
              <span className={`text-gray-500 ${
                needsCompactLayout ? 'text-lg' : 'text-xl'
              }`}>/ {steps.length}</span>
            </div>

            <h2 className={`font-bold tracking-tight ${
              needsCompactLayout ? 'text-4xl lg:text-5xl' : 'text-fluid-6xl'
            }`}>
              {steps[activeStep].title}
            </h2>

            <p className={`text-gray-300 leading-relaxed ${
              needsCompactLayout ? 'text-lg lg:text-xl' : 'text-fluid-xl'
            }`}>
              {steps[activeStep].description}
            </p>

            <div className={`border-t border-gray-800 overflow-visible ${
              needsCompactLayout ? 'pt-6 pb-4' : 'pt-8 pb-5'
            }`}>
              <div className={`font-bold text-gradient leading-tight overflow-visible ${
                needsCompactLayout ? 'text-5xl' : 'text-6xl'
